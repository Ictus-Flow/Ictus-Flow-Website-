  ...
  return (
    <section id="case-studies" className="py-24 bg-slate-950 relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <RevealOnScroll>
          <div className="mb-12">
            <h2 className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-400 font-black tracking-widest uppercase text-sm mb-2">
              CASE STUDIES
            </h2>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {cases.map((study, idx) => (
            <RevealOnScroll key={idx} className={`delay-[${idx * 150}ms]`}>
              <div
                className={`bg-white/5 backdrop-blur-xl rounded-2xl overflow-hidden border border-white/10 flex flex-col group ${study.border} transition-all duration-500 hover:-translate-y-2 shadow-[0_8px_32px_0_rgba(0,0,0,0.37)]`}
              >
                ...
                <div className="p-8 flex-1 flex flex-col relative -mt-4">
                  <div
                  ...
                  <div className="mb-6">
                    <span className="inline-block py-1 px-3 rounded bg-black/30 text-xs font-bold uppercase tracking-wider text-slate-400 group-hover:text-white transition-colors mb-3 backdrop-blur-md">
                      {study.client}
                    </span>
                    <h4 className="text-2xl font-bold text-white group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-slate-300 transition-all">
                      {study.title}
                    </h4>
                  </div>
                  <p className="text-slate-300 text-sm mb-8 flex-1 leading-relaxed group-hover:text-white font-medium relative z-10">
                    {study.desc}
                  </p>
                  <div className="pt-6 border-t border-white/10 group-hover:border-white/20 relative z-10">
                    <p
                      ref={study.ref}
                      className={`bg-gradient-to-r ${study.gradient} bg-clip-text text-transparent font-black text-3xl opacity-90`}
                    >
                      {study.resultVal}
                      {study.resultSuffix}
                    </p>
